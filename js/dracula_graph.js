...
Graph.Renderer.Raphael.prototype = {
    ...
    translateReverse: function(point) {
      return [(point[0] - this.radius) / this.factorX + this.graph.layoutMinX,
              (point[1] - this.radius) / this.factorY + this.graph.layoutMinY]
    },
    ...
    drawNode: function(node) {
        ...
        var ellipse = r.ellipse(0, 0, 40, 25)
                          .attr({fill: color, stroke: color, "stroke-width": 2, cursor: 'move'});
        /* set DOM node ID */
        ellipse.node.id = node.label || node.id;
        ellipse.set = shape;
        shape.ellipse = ellipse;
        shape.push(ellipse);
        ellipse.attr({"fill-opacity": .6});
        
        /* render ellipse */
        var text = r.text(0, 0, node.label || node.id)
                      .attr({cursor: 'move'});
        text.set = shape;
        shape.push(text);
        ...
        for (var i = 0; i < positions.length; i++) {
          var pos = positions[i];
          var terminal = r.ellipse(pos.x, pos.y, 5, 5).attr({fill: '#555', "fill-opacity": 0, "stroke-width": 0});
          terminal.set = shape;
        ...
        shape.mouseover(function() {
          terminals.animate({"fill-opacity": 1}, 200);
        });
        shape.mouseout(function() {
          terminals.animate({"fill-opacity": 0}, 200);
        });
        ...
        terminals.mousedown(function(event) {
          ...
          function updateEdge(event) {
            ...
            edge = self.r.connection(ellipse, {x: x, y: y, width: 1, height: 1});
          }
