...
$(function() {
  ...
  window.RemoteSelectFromMany = Widget.extend({
    ...
    update: function() {
      ...
      $(this.el).find('option[value=' + selected + ']').attr({selected: 'selected'});
...
$(function() {
  ...
  window.RowView2 = Backbone.View.extend({
    ...
    render: function() {
      ...
      $('<td class="delete"><a class="delete" href="javascript:void(0)">&nbsp;</a></td>')
        .appendTo(this.el)
  ...
  window.MatrixView = Backbone.View.extend({
    initialize: function(options) {
      ...
      this.newDialog = DialogView2.extend({
        ...
        definition: [{ label: 'Label', widget: TextInput.extend({ field: 'label' }) }],
    ...
    update: function() {
      ...
      var tr = $('<tr/>').appendTo(this.thead);
      $('<th/>').appendTo(tr);
      $('<th/>').text(this.prettyName.replace(/^(.)/, function(m) { return m.toUpperCase(); })).appendTo(tr);
      ...
      self.collection.each(function(variable) {
        ...
        $('<th class="delete"><a class="delete" href="javascript:void(0)">&nbsp;</a></th>')
          .appendTo(tr)
          .find('a')
          .click(function() {
            variable.destroy();
        ...
        input
          .val(variable.get('label'))
          .change(function() {
            variable.save({'label': input.val()});
          });          
      ...
      var tr = $('<tr class="new-item"/>').appendTo(this.tfoot);
      $('<th />').attr({colspan: 1}).appendTo(tr);
      var th = $('<th/>').attr({colspan: 1 + actionSituations.length}).appendTo(tr);
...
$(function() {
  generateViews('role', [
    { label: 'Label',                widget: TextInput.extend({ field: 'label' }) },
    { label: 'Objective',           widget: TextInput.extend({ field: 'objective' }) },
    { label: 'Sub-objectives¹',      widget: TextInput.extend({ field: 'sub_objectives' }) },
    { label: 'Institutions',        widget: RemoteSelect.extend({ field: 'institutions', optionCollection: 'institutions', optionLabelField: 'label', multiple: true, newDialog: 'InstitutionDialogView' }) },
  ...
  generateViews('institution', [
    { label: 'Label',           widget: TextInput.extend({ field: 'label' }) },
    { label: 'Attributes',     widget: RemoteSelect.extend({ field: 'roles', optionCollection: 'roles', optionLabelField: 'label', multiple: true, newDialog: 'RoleDialogView' }) },
    { label: 'Deontic type',   widget: Select.extend({ field: 'deontic_type', choices: { none: 'None', permission: 'Permission', obligation: 'Obligation', prohibition: 'Prohibition' } }) },
    ...
    { label: 'Institutional type', widget: Computed.extend({ field: 'instititional_type' } ) },
  ]);
  ...
  generateViews('agent', [
    ...
    { label: 'Physical components', widget: RemoteSelect.extend({ field: 'components', optionCollection: 'components', optionLabelField: 'label', multiple: true, newDialog: 'ComponentDialogView' }) },
    { label: 'Possible roles',          widget: RemoteSelect.extend({ field: 'roles', optionCollection: 'roles', optionLabelField: 'label', multiple: true, newDialog: 'RoleDialogView' }) },
    { label: 'Intrinsic capabilities¹', widget: TextInput.extend({ field: 'intrinsic_capability' }) },
    { label: 'Decision making criteria¹', widget: TextInput.extend({ field: 'decision_making_behavior' }) },
  ]);

  generateViews('component', [
    { label: 'Label',           widget: TextInput.extend({ field: 'label' }) },
    { label: 'Properties¹',     widget: TextInput.extend({ field: 'properties' }) },
    { label: 'Type',           widget: Select.extend({ field: 'type', choices: { open: 'Open', fenced: 'Fenced' } }) },
  ...
  generateViews('action', [
    { label: 'Action situation',         widget: RemoteSelect.extend({ field: 'actionSituation', optionCollection: 'actionSituations', optionLabelField: 'label', multiple: false, newDialog: ActionSituationDialogView }) },
    { label: 'Roles', widget: RemoteSelect.extend({ field: 'roles', optionCollection: 'roles', optionLabelField: 'label', multiple: true, newDialog: 'RoleDialogView' }) },
    { label: 'Action body',    widget: RemoteSelectFromMany.extend({ multiple: false, field: 'body', sources: [
      { optionCollection: 'components', optionField: 'behaviors' },
      { optionCollection: 'roles', optionField: 'institutional_capabilities' },
      { optionCollection: 'agents', optionField: 'intrinsic_capability' },
    ]}) },
    { label: 'Physical components', widget: RemoteSelect.extend({ field: 'components', optionCollection: 'components', optionLabelField: 'label', multiple: true, newDialog: 'ComponentDialogView' }) },
    { label: 'Institutional statement', widget: RemoteSelect.extend({ field: 'institutions', optionCollection: 'institutions', optionLabelField: 'label', multiple: true, newDialog: 'InstitutionDialogView' }) },
  ...
  generateViews('roleEnactment', [
    { label: 'Agent',      widget: RemoteSelect.extend({ field: 'agent', optionCollection: 'agents', optionLabelField: 'label', multiple: false }) },
    { label: 'Action',     widget: RemoteSelect.extend({ field: 'actionSituation', optionCollection: 'actionSituations', optionLabelField: 'label', multiple: false }) },
    { label: 'Role',       widget: RemoteSelect.extend({ field: 'role', optionCollection: 'roles', optionLabelField: 'label', multiple: false }) },
  ]);
});
  ...
  window.ConnectionDiagramView = Backbone.View.extend({
    ...
    render: function() {
      ...
      g.bind('changeedgelabel', function(args) {
        ...
        from.setConnection(to, {label: args.label});
        self.delayedUpdate();
      });
