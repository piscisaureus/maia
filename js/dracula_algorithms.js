...
function bellman_ford(g, source) {
  ...
  g.nodes[n].distance = Infinity; /* predecessors are implicitly null */
  source.distance = 0;
  ...
  for (var i = 1; i < g.nodes.length; i++) /* for each edge */
...
/* get the node with the smallest distance
  ...
  while (q.min() != undefined) { /* remove the latest */
    node = q.extractMin();
    node.optimized = true;
...
/* Note: Usually, the initialisation is done by getting the edge weights
  ...
  for (k in g.nodes) {
    for (i in g.nodes) {
      ...
      if (path[i][j] > path[i][k] + path[k][j]) {
        path[i][j] = path[i][k] + path[k][j]; /* Step 2.b: remember the path */
        next[i][j] = k;
      }
    }
  }
  ...
  function getPath(i, j) {
    ...
    else return getPath(i, intermediate).concat([intermediate]).concat(getPath(intermediate, j));
  }
...
/* Helper function to swap elements with their parent 
  ...
  function bubble_up(i) {
    var p = parent(i);
    while ((p >= 0) && (tree[i][key] < tree[p][key])) { /* swap with parent */
      tree[i] = tree.splice(p, 1, tree[i])[0]; /* go up one level */
      i = p;
      p = parent(i);
    }
  }
...
function quickSort(arr) { /* recursion anchor: one element is always sorted */
  if (arr.length <= 1) return arr; /* randomly selecting some value */
...
function selectionSort(arr) { /* recursion anchor: one element is always sorted */
  ...
  arr.splice(index, 1); /* assemble result and sort recursively (could be easily done iteratively as well)*/
...
function mergeSort(arr) { /* merges two sorted arrays into one sorted array */

  function merge(a, b) { /* result set */
    var c = []; /* as long as there are elements in the arrays to be merged */
    while (a.length > 0 || b.length > 0) { /* are there elements to be merged, if yes, compare them and merge */
      var n = a.length > 0 && b.length > 0 ? a[0] < b[0] ? a.shift() : b.shift() : b.length > 0 ? b.shift() : a.length > 0 ? a.shift() : null; /* always push the smaller one onto the result set */
      n != null && c.push(n);
    }
    return c;
...
function kmp(p, t) {
  ...
  function prefix(p) { /* pi contains the computed skip marks */
    var pi = [0],
        k = 0;
  ...
  for (var i = 0; i < t.length; i++) { /* jump forward as long as the character doesn't match */
...
function topological_sort_helper(g) {
  ...
  var node;
  for (i in g.nodes) {
    if (g.nodes[i].deleted) continue; //Bad style, meh
    var incoming = false;
    for (j in g.nodes[i].edges) {
      if (g.nodes[i].edges[j].target == g.nodes[i] && g.nodes[i].edges[j].source.deleted == false) {
        incoming = true;
        break;
      }
    }
